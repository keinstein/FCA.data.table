...
"_derive_2D_1D3" <- function(cxt, given, names) {
    ...
    n = unlist(names[1:2])
    (cxt[given,.(list(list(eval(as.name(names[3]))))),by=eval(n)]
     [,Reduce(intersect2,V1,V1[[1]][[1]])])
}
...
#' @return a an attribute representing the condition/attribute pairs that
...
#' @return a an attribute representing the object/attribute pairs that
